...
Let's take a moment here and imagine that I gave this article a more creative title. Thanks!
...
`for`-loops, but where it's possible, comprehensions can improve readability and clarify the intent
very well. We'll see how this works out in some examples in later part of this article.

I'm writing this article to share all I've learnt with regards to comprehensions, specifically in
Python 3. Note that some of the features described here, especially `set` and `dict` comprehensions,
...
Let's go over the basic syntax form for starters. It mainly contains three parts. The result
...
argument of the `range` builtin, but bear with me for lacking in creativity for examples!
{: .note }

## Basic examples

TODO: Some examples? Like <https://cmdlinetips.com/2018/01/5-examples-using-dict-comprehension/>
...
The syntax is almost exactly similar to list comprehensions. The difference is that we use braces
for set and dict comprehensions, where we use square brackets for list comprehensions. The looping
and filtering constructs behave the same way. The result expression behaves the same way for set
comprehensions, but for dict comprehensions, we have to provide two expressions, the key and the
value, separate by a colon. Let's look at some examples:
...
comprehension. Notice that the only difference in syntax, is the surrounding bracket type.
...
TODO: Check set and dict comprehensions on Python 2.7

## Multiple Looping Constructs

In the previous examples, we've only used one looping construct. However, it is possible to use
more than one looping constructs. This works very similar to using a nested `for`-loop. Let's look
at an example:
...
This output is easy to visualize if you see the two `for`-loops as forming a nested loop. The
following is a reproduction of the above, without comprehensions:
...
becomes obvious if you visualize this as the nested `for`-loop illustrated above. This becomes
important when using generators or iterators that work as single-pass, like `map` objects, or file
objects (for which, we'll need `.seek`), or even `range` objects. Check out the following example to
see what I mean:
...
You're not likely to encounter this in real-world code, but it's good to know should I end up facing
it.
...
can be done if we also make use of the `filter` builtin. So, the following comprehension expression:
...
Obviously, the comprehension reads much better, but I'd urge you to not throw away the `map` and
`filter` builtins. They have their place and sometimes, code using them can read much better than
comprehensions. Check out my [article on `map` function][map-article] for such examples and other
rationales.
...
which can be used to aggregate values in collections. I won't go into details of how this can be
used, but I will show how such an affect can be reproduced with comprehensions.
...
discomfort to the fact that this is a new operator and like anything in life, needs some getting
used to. However, since it's new in version 3.8, it's probably best to keep away from it in
...
They'll most certainly be less performant (and even less obvious to the reader of such code), but
...
to me or this article as inspiration. This is a sad little experiment and it should stay that way.

## Generator Expressions

When comprehensions are wrapped in square brackets or braces, the result is a fully realized
collection, like a list or a set. However, when not wrapped as such, or when wrapped with
...
Generator expressions were introduced in [PEP-289](https://www.python.org/dev/peps/pep-0289/) and
contains a lot of examples. I recommend reviewing it for some cool use cases, which I won't
reproduce here.

Once small note regarding passing generator expressions as an argument to functions is that, make it
...
get an error that the generator expression is not parenthesized. Check out the following example if
...
In the first case, the generator expression is the *only* argument to `sorted`, so it didn't need to
be parenthesized. In the second and third cases, we were passing the `key` argument as well. So in
these cases, the generator expression needs to be parenthesized.
...
As another best practice, please strive to have no side effects in your comprehension result
expressions. Check out the following example to see what I mean:
...
Let's see the stack they print and make sure they produce the same output:
...
The stack shows the file as `"<stdin>"` because I defined the functions within the REPL session. If
they were in an actual file, we'd get the file name there.
{: .note }
...
<textarea id=loopCodeEl></textarea>
<style>
#converterBox {
    display: flex;
    flex-wrap: wrap;
...
all-the-time. It's important to understand them well if you're doing more than the trivial single
