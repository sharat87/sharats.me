...
in some ways. A big chunk of the reason I'm productive on Windows is [AutoHotkey][]. I will document
my automations and experiences over the years in this article / series.
...
good, something I haven't found in any other general purpose programming language ([AutoIt][] may
come close, but I've never tried it so can't speak for it).

The language itself seems a bit flaky around it's syntax and some of the constructs, but once we get
used to them, there's a powerful engine underneath it. That, combined with the well-written
documentation, makes AutoHotkey a must-have tool for any Windows power user.

> All reasonably advanced technology is indistinguishable from magic.

In this article (or series), I'll share the hotkeys I use, how I came about them and how they
improve my workflow. It is not a beginner's AutoHotkey tutorial, that would be the official
documentation or the many other resources available online.

A lot of the stuff in this article is made possible by a lot of help from all over the internet, and
especially the AutoHotkey forums. Due to most of it being at least a few years old, I won't be able
to put up exact source links. Thank you everyone!
{: .note }
...
Most of this I learned to be a best practice from the documentation and from the forums.
...
I have exactly one AutoHotkey script running at all times, called `master.ahk`, which either
contains everything I use or `#Include`s stuff from other AutoHotkey scripts.

    :::ahk {file: master.ahk}
    #+r::Reload

I include the above in my `master.ahk` and when I hit <kbd>#+r</kbd>, the changes in `master.ahk`
and any included scripts are reloaded.
...
<kbd>Escape</kbd> key, the focus is not returned to the window that originally had focus. The
workaround for me has been to do <kbd>Alt+Tab</kbd> a couple of times and we're back at work.

It's still arguable how useful this solution is. The calendar that pops up itself has very limited
functionality. The most annoying this is probably that I spend a few seconds selecting the month I
want to look at and accidentally click on another window and that calendar is gone! After a lot of
swearing, I attempted to solve this problem and build
[calendar.sharats.me](https://calendar.sharats.me). It's a no-backend, no-login, pure-JS, just a
calendar to look at, and mark dates to top. Do check it out! Thanks.
{: .note }
...
reduce me to swearing again. Don't get me wrong, I know we can turn this behaviour off by disabling
Aero or some other setting and I can even agree that this feature can be useful. But to me, firstly,
I hardly keep anything on my Desktop so even it's existence is quite useless to me. Secondly, even
if I wanted to look at the desktop, it's a quick <kbd>#d</kbd> away, which is much faster
considering my fingers are almost always on the keyboard.

So I decided to hide the "Show Desktop" button.

    :::ahk
    Control, Hide, , TrayShowDesktopButtonWClass1
        , ahk_class Shell_TrayWnd ahk_exe explorer.exe

This doesn't reclaim the space occupied by the button, but the button disappears and my problems go
away, so, I'm not complaining.

### Type Clipboard Contents

Remember how some websites (especially bank websites) disallow pasting values into inputs. This is
extremely annoying when using a password manager or otherwise, when I want to just paste something.
I've tried several different solutions to this, and the current answer I have with AutoHotkey has
served me the best.
...
don't. For example, the photo viewer, font viewer, the playlist in VLC etc. Then there's another set
of windows that I found myself trying to close by hitting <kbd>^w</kbd>. I think that intuition came
from it being the key to close tabs in Firefox and Chrome. Either way, I needed these keys to act
the way I was expecting them to.

There's two parts to the solution to this. First, we define the hotkeys to close the windows:
...
active in the lock screen (where I hit <kbd>Ctrl+A</kbd> often). But since SharpKeys modifies to
registry to achieve what it does, the remapping works even in the lock screen.

Yet, sometimes I miss the original functionality of <kbd>Caps Lock</kbd> key. So I created the
...
than using the original <kbd>Caps Lock</kbd> key, because I get an overlay that alerts me that Caps
Lock is turned on.

## Inserting Snippets

Inserting snippets is an idea where a long and often used string is inserted by a rather short
sequence of keys. In AutoHotkey, this is *usually* done using [hotstrings][]. Hotstrings word quite
mostly okay for this actually, but they don't work on every application. For me particularly, I
needed them to be working with GVim, which they weren't. So, with a lot of help from the Internet, I
came up with a solution.

Instead of hotstrings, I'll use a hotkey that summons a always-on-top window with a list of keys and
their expansions. When this window is focused, I can hit one of those keys and the windows is
...
For example, I want all command line windows to always be moved to the top right corner. As another
example, there's some windows that open up with a window size equal to the whole screen, but are not
maximized. This one is particularly annoying since I have a habit of throwing my mouse to the top
right corner and clicking to close the window. But since this window is not maximized, I end up
accidentally closing the window behind. So, I want such windows to be maximized as soon as they
open.
...
I don't know if there's a simpler solution to achieve all this, but has been working well for me so
far.

There are other methods to achieve this without polling and I encourage you to try them out if
you're not comfortable with polling, like with using `RegisterShellHookWindow`. In my experience,
such solutions seemed to miss some windows and were able to catch only a small limited set of the
windows there were opening. So I went with polling, which was less efficient, but more reliable.

## Mess with Images in Clipboard

This is a little trick that's powered by [ImageMagick][]. I add a menu item in the tray icon's
context menu called `"Add border to image in clipboard"`, which basically explains what this does!
...
This one probably makes sense if your fingers are used to hitting [Vim][]'s hotkeys. I wanted some
of Vim's simple hotkeys for navigating the document on Sumatra PDF (my PDF reader of choice on
Windows). The following snippet that I currently use, gets me <kbd>d</kbd> (like
Vim's <kbd>&lt;C-d&gt;</kbd>), <kbd>e</kbd> (like Vim's <kbd>&lt;C-u&gt;</kbd>), <kbd>n</kbd>,
...
be. This sentiment shows itself a lot when dealing with AutoHotkey code. But it works, and it works
really well.
...
I hope to publish a sequel to this article in a week or two, so, stay tuned or subscribe!
